...
from grocery import GridAndBoxLayoutScreen
...
class LoginScreen1234567(App): #LoginScreenApp --> LoginScreenApp --> Remove the name APP -> LoginScreen
    def build(self):
        ...
        self.screen_manager.add_widget(LoginScreenGrid(name="login"))
        self.screen_manager.add_widget(GridAndBoxLayoutScreen(name="details"))
...
class LoginScreenGrid(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        grid.add_widget(Label(text="UserName", height=30, size_hint_y=None)) # 1
        self.username_text_object = TextInput(height=30, size_hint_y=None) # 3
        grid.add_widget(self.username_text_object)
        
        grid.add_widget(Label(text="Password", height=30, size_hint_y=None)) # 4
        self.password_text_obj = TextInput(password=True, height=30, size_hint_y=None)
        grid.add_widget(self.password_text_obj)
    ...
    def validate_user_name_and_password(self, kivy_instance):
        ...
        if self.username_text_object.text == dummy_user_name and self.password_text_obj.text == dummy_password:
            self.manager.current = "details" # REDIRECTION
            print("HI")
        else:
